submodule (unary_operator_checker) imp_check_unary_half
    ...
    module procedure check_unary_half
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_half.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -32, 32
            ...
            trial%r_op = trial%r_x / 2
            trial%e_op = half(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('half')

                error stop ': half(x) @ eml is NaN.'
        ...
        if (flag) call record%display('half')
