submodule (unary_operator_checker) imp_check_unary_atan
    ...
    module procedure check_unary_atan
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_atan.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -64, 64
            ...
            trial%r_op = atan(trial%r_x)
            trial%e_op = atan(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('atan')

                error stop ': atan(x) @ eml is NaN.'
        ...
        if (flag) call record%display('atan')
