submodule (unary_operator_checker) imp_check_unary_acosh
    ...
    module procedure check_unary_acosh
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_acosh.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = 8, 72
            ...
            trial%r_op = acosh(trial%r_x)
            trial%e_op = acosh(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('acosh')

                error stop ': acosh(x) @ eml is NaN.'
        ...
        if (flag) call record%display('acosh')
