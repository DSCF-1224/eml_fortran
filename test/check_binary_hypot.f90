submodule (binary_operator_checker) imp_check_binary_hypot
    ...
    module procedure check_binary_hypot
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_binary_hypot.dat' , &!
            status  = 'replace' &!
        )
        ...
        do j = -16, 16
            ...
            trial%r_op = hypot(trial%r_x, trial%r_y)
            trial%e_op = hypot(trial%e_x, trial%e_y)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('hypot')

                error stop ': hypot(x,y) @ eml is NaN.'
        ...
        if (flag) call record%display('hypot')
