submodule (unary_operator_checker) imp_check_unary_cosh
    ...
    module procedure check_unary_cosh
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_cosh.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -32, 32
            ...
            trial%r_op = cosh(trial%r_x)
            trial%e_op = cosh(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('cosh')

                error stop ': cosh(x) @ eml is NaN.'
        ...
        if (flag) call record%display('cosh')
