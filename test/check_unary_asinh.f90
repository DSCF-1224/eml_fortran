submodule (unary_operator_checker) imp_check_unary_asinh
    ...
    module procedure check_unary_asinh
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_asinh.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -64, 64
            ...
            trial%r_op = asinh(trial%r_x)
            trial%e_op = asinh(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('asinh')

                error stop ': asinh(x) @ eml is NaN.'
        ...
        if (flag) call record%display('asinh')
