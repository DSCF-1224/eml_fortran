submodule (unary_operator_checker) imp_check_unary_atanh
    ...
    module procedure check_unary_atanh
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_atanh.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -64, 64
            ...
            trial%r_op = atanh(trial%r_x)
            trial%e_op = atanh(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('atanh')

                error stop ': atanh(x) @ eml is NaN.'
        ...
        if (flag) call record%display('atanh')
