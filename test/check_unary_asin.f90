submodule (unary_operator_checker) imp_check_unary_asin
    ...
    module procedure check_unary_asin
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_asin.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -100, 100
            ...
            trial%r_op = asin(trial%r_x)
            trial%e_op = asin(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('asin')

                error stop ': asin(x) @ eml is NaN.'
        ...
        if (flag) call record%display('asin')
