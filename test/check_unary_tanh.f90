submodule (unary_operator_checker) imp_check_unary_tanh
    ...
    module procedure check_unary_tanh
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_tanh.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = -32, 32
            ...
            trial%r_op = tanh(trial%r_x)
            trial%e_op = tanh(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('tanh')

                error stop ': tanh(x) @ eml is NaN.'
        ...
        if (flag) call record%display('tanh')
