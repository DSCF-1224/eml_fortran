submodule (unary_operator_checker) imp_check_unary_tan
    ...
    module procedure check_unary_tan
        ...
        do i = -40, 40
            ...
            trial%r_x  = 0.03125_real64 * i
            trial%e_x  = trial%r_x

            trial%r_op = tan(trial%r_x)
            trial%e_op = tan(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('tan')

                error stop ': tan(x) @ eml is NaN.'
        ...
        if (flag) call record%display('tan')
