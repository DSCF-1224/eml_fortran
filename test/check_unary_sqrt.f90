submodule (unary_operator_checker) imp_check_unary_sqrt
    ...
    module procedure check_unary_sqrt
        ...
        open( &!
            newunit = file_unit , &!
            file    = 'test/check_unary_sqrt.dat' , &!
            status  = 'replace' &!
        )
        ...
        do i = 0, 64
            ...
            trial%r_op = sqrt(trial%r_x)
            trial%e_op = sqrt(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('sqrt')

                error stop ': sqrt(x) @ eml is NaN.'
        ...
        if (flag) call record%display('sqrt')
