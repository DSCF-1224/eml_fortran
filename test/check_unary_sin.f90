submodule (unary_operator_checker) imp_check_unary_sin
    ...
    module procedure check_unary_sin
        ...
        do i = -160, 160
            ...
            trial%r_op = sin(trial%r_x)
            trial%e_op = sin(trial%e_x)



            if ( ieee_is_nan(trial%e_op) ) then

                call trial%display('sin')

                error stop ': sin(x) @ eml is NaN.'
        ...
        if (flag) call record%display('sin')
